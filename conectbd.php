...
/* Основные функции вывода */ 
	...
	function checkPerc($table='master_menu_perc'){
		$mysqli = mysqli_connect(DB_HOST,DB_USER, DB_PASSWORD, DB_NAME);
		if (mysqli_connect_errno($mysqli)) {
    		echo "Не удалось подключиться к MySQL: " . mysqli_connect_error();
		}
	...
	function insertPerc ( $table='master_menu_perc'){
		$mysqli = mysqli_connect(DB_HOST,DB_USER, DB_PASSWORD, DB_NAME);
		if (mysqli_connect_errno($mysqli)) {
    		echo "Не удалось подключиться к MySQL: " . mysqli_connect_error();
		}
		mysqli_query($mysqli,"INSERT INTO $table (`id`, `flag`) VALUES (NULL, 1)");	
	}
	...
	function DeletePerc ($table='master_menu_perc'){
		$mysqli = mysqli_connect(DB_HOST,DB_USER, DB_PASSWORD, DB_NAME);
		if (mysqli_connect_errno($mysqli)) {
    		echo "Не удалось подключиться к MySQL: " . mysqli_connect_error();
		}
		mysqli_query($mysqli,"DELETE FROM `master_menu_perc` WHERE 1");	
